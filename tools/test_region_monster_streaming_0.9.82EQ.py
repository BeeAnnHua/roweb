...
check(config.get('version')=='0.9.82EQ','spawn config version')
check(manifest.get('version')=='0.9.82EQ','manifest version')
...
check(ai_modes.get('version')=='0.9.82EQ','AI mode version')
...
check(versions=={'0.9.82EQ'},f'cache versions {versions}')
check('RO_WEB V0.9.82EQ' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EQ','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
