...
check(config.get('version')=='0.9.82EP','spawn config version')
check(manifest.get('version')=='0.9.82EP','manifest version')
...
check(ai_modes.get('version')=='0.9.82EP','AI mode version')
...
check(versions=={'0.9.82EP'},f'cache versions {versions}')
check('RO_WEB V0.9.82EP' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EP','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
