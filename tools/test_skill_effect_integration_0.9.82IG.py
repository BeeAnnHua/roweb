...
check('index_loads_v92_runtime','skill_effect_runtime_v92.js?v=0.9.82IG' in index)
...
check('version_updated','RO_WEB 0.9.82IG' in index and 'const RO_WEB_VERSION = "0.9.82IG";' in (ROOT/'js/game.js').read_text(encoding='utf-8'))
...
vm=subprocess.run(['node',str(ROOT/'tools/test_skill_effect_runtime_0.9.82IG.js')],capture_output=True,text=True)
check('node_vm_runtime_selftest',vm.returncode==0,{'stdout':vm.stdout[-3000:],'stderr':vm.stderr[-1000:]})
result={'version':'0.9.82IG','pass':not errors,'summary':{'skills':len(skill_rows),'effects':len(effect_rows),'strictEffectJson':strict_count,'pngDependencyReferences':dep_count,'passiveRuntimeSkills':len(passive_ids),'candidatePassiveIntersection':len(ids&passive_ids),'checksPassed':sum(1 for x in checks.values() if x['pass']),'checksTotal':len(checks)},'checks':checks,'errors':errors}
out=ROOT/'TEST_REPORT_0.9.82IG_SKILL_EFFECT_RUNTIME.json'
out.write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
(ROOT/'TEST_REPORT_0.9.82IG_SKILL_EFFECT_RUNTIME.txt').write_text('\n'.join([
 'RO_WEB 0.9.82IG V92 Skill Effect Runtime Test',
 '='*56,
