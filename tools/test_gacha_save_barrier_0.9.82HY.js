...
context.RO_WEB_DATA["data/item_boxes.json"]={boxes:{}};
vm.runInContext(fs.readFileSync(path.join(ROOT,"js/item_box_runtime.js"),"utf8"),context,{filename:"js/item_box_runtime.js"});
vm.runInContext(fs.readFileSync(path.join(ROOT,"js/item_batch_open_runtime.js"),"utf8"),context,{filename:"js/item_batch_open_runtime.js"});

const countGacha=()=>Number(context.findInventoryItemById(gachaId)?.count||0);
const MAIN="ro_web_save_v0_9_19_ui_scroll_quickbar";
(async()=>{
  assert.strictEqual(context.MvpGachaRuntime.version,"0.9.82HY");
  ...
  const report={version:"0.9.82HY",passed:18,failed:0,checks:{queued4000:true,manualDrain:true,writerReclaim:true,localReadback:true,durableReadback:true,quotaFallback:true},logs:logs.slice(-8)};
  fs.writeFileSync(path.join(ROOT,"tools/test_gacha_save_barrier_report_0.9.82HY.json"),JSON.stringify(report,null,2)+"\n");
