...
with sync_playwright() as p:
    ...
    assert page.locator('.map-monster-drop-footer .map-monster-return-list').count()==1
    assert page.locator('.map-monster-drop-footer .map-monster-return-map').count()==1
    ...
    # Reopen and verify footer return-to-map.
    page.evaluate('showMapMonsterDistributionTooltip(maps[0],document.getElementById("anchor"))')
    page.locator('.map-monster-distribution-row').tap()
    page.locator('.map-monster-drop-footer .map-monster-return-map').tap()
    assert page.locator('#map-monster-distribution-tooltip').is_hidden()
    ...
    browser.close()
print('PASS map viewer navigation, touch drag, close, enter map')
