...
def main():
    ...
    index_text = (root / "index.html").read_text(encoding="utf-8")
    rare_tag = './js/rare_item_announcement_runtime.js?v=0.9.82HR'
    if rare_tag not in index_text: a.error("HR_RARE_RUNTIME_INDEX", rare_tag)
    else:
        try:
            rare_pos=index_text.index(rare_tag); gacha_pos=index_text.index('./js/mvp_gacha_runtime.js?v=0.9.82HR'); box_pos=index_text.index('./js/item_box_runtime.js?v=0.9.82HR')
