...
check(html.includes('death_revival_runtime.js?v=0.9.82HV'), '死亡 Runtime 以 HU 快取版本載入');
...
const report = {
  version:'0.9.82HV', suite:'death-revival-token-auto-revive-gacha',
  passed:passes.length, failed:failures.length, passes, failures
};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HV_DEATH_REVIVAL.json'),JSON.stringify(report,null,2)+'\n');
