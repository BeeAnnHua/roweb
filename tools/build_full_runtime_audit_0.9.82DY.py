...
full_skill=load('tools/full_skill_audit_0.9.82DY.json')
ra=load('tools/ra_skill_timing_audit_0.9.82DY.json')
action=load('tools/all_skill_timing_action_audit_0.9.82DY.json')
combat=load('tools/ra_renewal_combat_audit_0.9.82DY.json')
critical=load('tools/ra_renewal_skill_critical_flags_audit_0.9.82DY.json')
range_audit=load('tools/ra_skill_range_audit_0.9.82DY.json')
distance=load('tools/all_skill_distance_profile_audit_0.9.82DY.json')
pending_scope=load('tools/pending_skill_scope_audit_0.9.82DY.json')
...
result={
 ...
 'knownLimitations':[
   ...
   'Historical regression scripts that hard-code older timing assumptions are not authoritative for the DY Renewal timing layer; dedicated 0.9.82DY tests are authoritative.',
 ...
 'summary':{
   'status':'PASS' if not errors and not full_skill['errors'] and not ra['errors'] and not action['errors'] and combat['summary']['status']=='PASS' and not critical['errors'] and not range_audit['errors'] and not distance['errors'] and pending_scope['unexpectedPending']==0 else 'FAIL',
...
out=ROOT/'docs/FULL_SKILL_CHARACTER_RUNTIME_AUDIT_0.9.82DY.json'
