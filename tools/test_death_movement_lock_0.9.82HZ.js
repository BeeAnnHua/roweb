...
check(html.includes('./js/position_engine.js?v=0.9.82HZ') && html.includes('./js/death_revival_runtime.js?v=0.9.82HZ'),'HU 快取版本正確載入移動與死亡 Runtime');
...
const report={version:'0.9.82HZ',suite:'death-movement-lock',passed:passes.length,failed:failures.length,passes,failures};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HZ_DEATH_MOVEMENT_LOCK.json'),JSON.stringify(report,null,2)+'\n');
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HZ_DEATH_MOVEMENT_LOCK.txt'),`RO_WEB 0.9.82HZ Death Movement Lock\nPassed: ${passes.length}\nFailed: ${failures.length}\n${failures.join('\n')}\n`);
