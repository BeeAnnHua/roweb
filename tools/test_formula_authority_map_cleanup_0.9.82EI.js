...
const expectedWorldMaps=[
  'prontera_3x3_region_camera','geffen_3x3_region_camera','morocc_3x3_region_camera',
  'mjolnir_3x3_region_camera','rachel_3x3_region_camera','payon_3x3_region_camera',
  'juno_3x3_region_camera','umbala_3x3_region_camera','lighthouse_coast_3x3_region_camera',
  'veins_3x3_region_camera'
];
assert(maps.length===10,'ten independent world regions should be present');
assert(JSON.stringify(maps.map(m=>m.id))===JSON.stringify(expectedWorldMaps),'world region order/id mismatch');
assert(maps.every(m=>m.worldCamera&&m.worldScale===3&&m.worldWidth===4608&&m.worldHeight===4608),'world camera parity mismatch');
assert(maps.every(m=>m.noMonster===false&&m.monsterStreaming===true&&Array.isArray(m.monsters)&&m.monsters.length>0),'regional monster streaming must be active');
...
assert(authority&&authority.version==='0.9.82EH'&&authority.ruleset==='rAthena Renewal','formula authority metadata invalid');
...
console.log('PASS 0.9.82EI formula authority, removed-map cleanup and regional monster activation');
