...
checks.update({
  ...
  'status_watchdog':'function startAutoBattleWatchdog()' in battle and 'function recoverAutoBattleScheduler' in battle,
  'status_open_close_recovery':(ROOT/'js/status_system.js').read_text(encoding='utf-8').count('recoverAutoBattleScheduler(')>=3,
  'no_startup_retaliation':'requestManualRetaliationAgainstMonster(entity' not in (ROOT/'js/world_monster_test_runtime.js').read_text(encoding='utf-8'),
  'town_no_retaliation_suppression':'RO_WEB_SUPPRESS_PASSIVE_RETALIATION' not in battle,
})
report={'title':'RO_WEB 0.9.82II Chromium UI / Runtime Contract','pass':all(checks.values()),'checks':checks,'computedStyles':result}
(ROOT/'TEST_REPORT_0.9.82II_BROWSER_UI_CONTRACT.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
(ROOT/'TEST_REPORT_0.9.82II_BROWSER_UI_CONTRACT.txt').write_text('\n'.join([report['title'],'='*60,f"Result: {'PASS' if report['pass'] else 'FAIL'}",*[f"{'PASS' if v else 'FAIL'} - {k}" for k,v in checks.items()]])+'\n',encoding='utf-8')
