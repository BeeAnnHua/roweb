...
COMMANDS=[
 ...
 ['node','tools/test_release_cleanup_town_restore_map_tooltip_0.9.82FG.js'],
 ['node','tools/test_default_prontera_start_0.9.82FG.js'],
 ...
 ['node','tools/test_auto_battle_attack_heading_0.9.82FG.js'],
 ['node','tools/test_map_spawn_smoothing_0.9.82FG.js'],
 ...
 ['node','tools/test_map_quest_cast_buff_0.9.82FG.js'],
...
for cmd in COMMANDS:
 ...
 if cp.returncode:
  result={'version':'0.9.82FG','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82FG.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82FG','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82FG.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
