...
assert(game.includes('const RO_WEB_VERSION = "0.9.82FI";'),'Runtime version must be FH');
assert([...index.matchAll(/\?v=([^"']+)/g)].every(m=>m[1]==='0.9.82FI'),'All entry cache keys must be FH');
console.log(JSON.stringify({version:'0.9.82FI',status:'PASS',townRestore:true,namesOnlyTooltip:true,mobileEmbedded:true},null,2));
