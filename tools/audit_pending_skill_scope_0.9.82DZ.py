...
result={
 'version':'0.9.82DZ','totalPending':len(pending),'expandedJobsDeferred':len(expanded),'intentionalSystemPending':len(intentional),'unexpectedPending':len(unexpected),
...
out=ROOT/'tools/pending_skill_scope_audit_0.9.82DZ.json';out.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
