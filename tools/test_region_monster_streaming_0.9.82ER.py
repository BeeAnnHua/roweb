...
check(config.get('version')=='0.9.82ER','spawn config version')
check(manifest.get('version')=='0.9.82ER','manifest version')
...
check(ai_modes.get('version')=='0.9.82ER','AI mode version')
...
check(versions=={'0.9.82ER'},f'cache versions {versions}')
check('RO_WEB V0.9.82ER' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82ER','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
