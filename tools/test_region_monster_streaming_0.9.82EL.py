...
check(config.get('version')=='0.9.82EL','spawn config version')
check(manifest.get('version')=='0.9.82EL','manifest version')
...
check(ai_modes.get('version')=='0.9.82EL','AI mode version')
...
check('V0.9.82EL - RA regional monster streaming' in css,'stream CSS missing')
check('.world-monster-debug-cross' not in css,'debug cross CSS remains')
index=(ROOT/'index.html').read_text(encoding='utf-8')
versions=set(re.findall(r'[?&]v=([^"&]+)',index))
check(versions=={'0.9.82EL'},f'cache versions {versions}')
check('RO_WEB V0.9.82EL' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EL','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
