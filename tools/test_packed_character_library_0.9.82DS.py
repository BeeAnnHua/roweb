...
def main() -> int:
    ...
    if len(all_atlas_json) != 876:
        errors.append(f"atlas JSON on disk={len(all_atlas_json)} expected=876")
    if len(all_png) != 966:
        errors.append(f"PNG on disk={len(all_png)} expected=966")
    if old_fixed_png:
    ...
    print("RO_WEB 0.9.82DS Packed Character Library Test")
    print("=" * 58)
