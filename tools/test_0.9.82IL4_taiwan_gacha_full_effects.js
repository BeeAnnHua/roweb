...
check('index cache IL4',text('index.html').includes('card_runtime.js?v=0.9.82IL4')&&text('index.html').includes('quick_slots.js?v=0.9.82IL4'));

const failed=checks.filter(x=>!x.passed);
const report={version:'0.9.82IL4',suite:'taiwan-gacha-full-equipment-card-runtime-audit',passed:checks.length-failed.length,failed:failed.length,checks,coverage};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82IL4_TAIWAN_GACHA_FULL_EFFECTS.json'),JSON.stringify(report,null,2)+'\n');
