...
check('<title>RO_WEB 0.9.82GB</title>' in index,'index version')
check('const RO_WEB_VERSION = "0.9.82GB"' in game,'game version')
check(set(re.findall(r'\?v=([^"\']+)',index))=={'0.9.82GB'},'cache version consistency')
check('js/mvp_gacha_runtime.js?v=0.9.82GB' in index,'MVP gacha Runtime loaded')
check(index.index('js/item_instance_ui.js?v=0.9.82GB') < index.index('js/mvp_gacha_runtime.js?v=0.9.82GB') < index.index('js/loot.js?v=0.9.82GB'),'gacha Runtime load order')
...
check(server.get('drop')==20000 and server.get('cardDrop')==100 and server.get('mapExclusiveDrop')==100,'drop master and category valves are configured',server)
check(load('data/items/cash.json')['14848']['icon']=='images/items/9525.webp','gacha uses ITEM 9525 icon')
check((ROOT/'images/items/14848.webp').read_bytes()==(ROOT/'images/items/9525.webp').read_bytes(),'gacha fallback image equals ITEM 9525 bytes')
...
check(tier_counts=={'normal':681,'boss':80,'mvp':149},'910 cards classified normal/Boss/MVP',tier_counts)
...
report={'version':'0.9.82GB','summary':{'checks':len(checks),'passed':sum(x['ok'] for x in checks),'failed':len(errors)},'errors':errors,'checks':checks}
(ROOT/'tools/test_report_0.9.82GB.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
