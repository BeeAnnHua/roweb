...
COMMANDS=[
 ...
 ['node','tools/test_formula_authority_map_cleanup_0.9.82EC.js'],
 ...
 ['node','tools/test_trait_stats_0.9.82EC.js'],
 ...
 ['node','tools/test_ra_renewal_skill_critical_flags_0.9.82EA.js'],
 ...
 ['python','tools/build_data_bundle_0.9.82EC.py'],
 ['python','tools/deep_health_check.py','.','--json-output','tools/deep_health_report_0.9.82EC.json'],
]
rows=[]
for cmd in COMMANDS:
 ...
 if cp.returncode:
  result={'version':'0.9.82EC','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82EC.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82EC','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82EC.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
