...
check('index_loads_v92_runtime','skill_effect_runtime_v92.js?v=0.9.82IB' in index)
...
check('version_updated','RO_WEB 0.9.82IB' in index and 'const RO_WEB_VERSION = "0.9.82IB";' in (ROOT/'js/game.js').read_text(encoding='utf-8'))
...
result={'version':'0.9.82IB','pass':not errors,'summary':{'skills':len(skill_rows),'effects':len(effect_rows),'strictEffectJson':strict_count,'pngDependencyReferences':dep_count,'passiveRuntimeSkills':len(passive_ids),'candidatePassiveIntersection':len(ids&passive_ids),'checksPassed':sum(1 for x in checks.values() if x['pass']),'checksTotal':len(checks)},'checks':checks,'errors':errors}
out=ROOT/'TEST_REPORT_0.9.82IB_SKILL_EFFECT_RUNTIME.json'
out.write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
(ROOT/'TEST_REPORT_0.9.82IB_SKILL_EFFECT_RUNTIME.txt').write_text('\n'.join([
 'RO_WEB 0.9.82IB V92 Skill Effect Runtime Test',
 '='*56,
