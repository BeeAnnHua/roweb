...
marker='<script src="./js/data_bundle.js?v=0.9.82GQ"></script>'
...
if not chrome:
    print(json.dumps({'version':'0.9.82GQ','passed':0,'total':1,'failed':['Chromium unavailable']},ensure_ascii=False))
    sys.exit(2)
profile=tempfile.mkdtemp(prefix='gq-cardinal-')
...
print(json.dumps({'version':'0.9.82GQ','passed':len(checks)-len(failed),'total':len(checks),'failed':failed,'checks':checks},ensure_ascii=False,indent=2))
