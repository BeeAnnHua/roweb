...
test('status window uses frozen combat snapshot and scheduler wake/watchdog', () => {
  ...
  assert(status.includes('recoverAutoBattleScheduler("status_window_open_before_render")'));
  assert(status.includes('recoverAutoBattleScheduler("status_window_close")'));
  assert(battle.includes('function startAutoBattleWatchdog()'));
  assert(battle.includes('function recoverAutoBattleScheduler(reason = "recovery")'));
  assert(battle.includes('finally {'));
  assert(read('js/world_monster_test_runtime.js').includes('不得自動啟動'));
  assert(battle.includes('now > autoBattleNextDueAt + 420'));
  assert(ui.includes('handleStatusWindowVisibilityChange(true)'));
});
...
(async()=>{
  ...
  const report={title:'RO_WEB 0.9.82II Regression',version:'0.9.82II',passed:tests.length-failed.length,failed:failed.length,tests};
