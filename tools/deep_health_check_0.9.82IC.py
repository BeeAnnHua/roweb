...
def main():
    ...
    try:
        ...
        # V92 effect JSON is intentionally lazy-loaded from the external Ready Library;
        # bundling 454 effect timelines into data_bundle.js would duplicate tens of MB.
        runtime_json=sorted(list((root/"data").rglob("*.json"))+[
            p for p in (root/"assets").rglob("*.json")
            if "assets/skill_effects/v92/" not in str(p.relative_to(root)).replace("\\","/")
        ])
    ...
    if hw_gacha.get("version") not in {"0.9.82HX", EXPECTED_VERSION}: a.error("GACHA_VERSION", str(hw_gacha.get("version")))
    ...
    hw_report = root / "TEST_REPORT_0.9.82HY_MVP_GACHA_PER_ITEM.json"
    ...
    hz_report = root / "TEST_REPORT_0.9.82HZ_RACE_MODIFIER_CANONICALIZATION.json"
    if not hz_report.is_file():
        a.error("HZ_RACE_REPORT_MISSING", hz_report.name)
    else:
        report = a.load(hz_report.name) or {}
        if int(report.get("failed", -1) if isinstance(report.get("failed"), int) else len(report.get("failed") or [])) != 0:
            a.error("HZ_RACE_REPORT_FAILED", str(report.get("failed")))
        if int(report.get("passed", 0)) < 60:
            a.error("HZ_RACE_REPORT_COUNT", str(report.get("passed")))
