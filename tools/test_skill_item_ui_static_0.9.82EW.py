...
assert versions and set(versions)=={'0.9.82EW'}, sorted(set(versions))
assert index.index('js/player.js?v=0.9.82EW') < index.index('js/item_instance_ui.js?v=0.9.82EW') < index.index('js/game.js?v=0.9.82EW')
...
assert client['version']=='0.9.82EW'
assert client['duplicateCardPrefixes']=={'2':'兩倍','3':'三倍','4':'四倍'}
assert len(client['cardPrefixNames'])>=1700
assert len(client['cardInfo'])>=1800
print(json.dumps({'version':'0.9.82EW','status':'PASS','cacheRefs':len(versions),'cardPrefixes':len(client['cardPrefixNames']),'cardDetails':len(client['cardInfo'])},ensure_ascii=False,indent=2))
