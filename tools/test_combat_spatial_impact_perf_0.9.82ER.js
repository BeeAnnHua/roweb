...
for(const token of ['RO_WEB_MONSTER_IMPACT_BATCH','propagateWorldMonsterAssistBatch(targets)','updateWorldMonsterHpBarFast','maxPerFrame: 24'])assert(battleSource.includes(token),`missing impact batch token ${token}`);
...
console.log(JSON.stringify({version:'0.9.82ER',status:'PASS',totalEntities:120,localResults:local.length,visitedEntities:stats.visitedEntities,visitedBuckets:stats.visitedBuckets,damageDom:'raw queue + 24/frame',impact:'single RAF + batched Assist'},null,2));
