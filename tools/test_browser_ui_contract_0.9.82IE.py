...
report={'title':'RO_WEB 0.9.82IE Chromium UI / Runtime Contract','pass':all(checks.values()),'checks':checks,'computedStyles':result}
(ROOT/'TEST_REPORT_0.9.82IE_BROWSER_UI_CONTRACT.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
(ROOT/'TEST_REPORT_0.9.82IE_BROWSER_UI_CONTRACT.txt').write_text('\n'.join([report['title'],'='*60,f"Result: {'PASS' if report['pass'] else 'FAIL'}",*[f"{'PASS' if v else 'FAIL'} - {k}" for k,v in checks.items()]])+'\n',encoding='utf-8')
