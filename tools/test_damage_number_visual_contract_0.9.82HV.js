...
check(html.includes('<title>RO_WEB 0.9.82HV</title>'),'HV title updated');
check(html.includes('./js/battle.js?v=0.9.82HV'),'battle cache version is HV');
...
const report={version:'0.9.82HV',suite:'damage-number-visual-contract',passed:passes.length,failed:failures.length,passes,failures};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HV_DAMAGE_NUMBER_VISUAL_CONTRACT.json'),JSON.stringify(report,null,2)+'\n');
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HV_DAMAGE_NUMBER_VISUAL_CONTRACT.txt'),`RO_WEB 0.9.82HV Damage Number Visual Contract\nPassed: ${passes.length}\nFailed: ${failures.length}\n${failures.join('\n')}\n`);
