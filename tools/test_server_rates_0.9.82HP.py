...
assert config['version']=='0.9.82HP',config.get('version')
...
assert bundled['version']=='0.9.82HP',bundled.get('version')
assert bundled['server']['rates']==expected,bundled['server']['rates']
print(json.dumps({'version':'0.9.82HP','passed':2,'failed':0,'rates':expected},ensure_ascii=False,indent=2))
