...
const result={version:'0.9.82DY',renewalOnly:true,counts:{flagged,implementedFlagged,implementedAttackFlagged,pendingFlagged,passiveFlagged,genericFallback,explicit},errors,rows,summary:{status:errors.length?'FAIL':'PASS',errors:errors.length}};
fs.writeFileSync(path.join(ROOT,'tools/ra_renewal_skill_critical_flags_audit_0.9.82DY.json'),JSON.stringify(result,null,2)+'\n');
