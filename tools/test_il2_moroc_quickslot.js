...
check('version IL2',cfg.version==='0.9.82IL2',cfg.version);
...
check('index cache version IL2',fs.readFileSync(path.join(ROOT,'index.html'),'utf8').includes('taiwan_gacha_runtime.js?v=0.9.82IL2'));

const failed=checks.filter(x=>!x.passed);
const report={version:'0.9.82IL2',suite:'420236-and-taiwan-gacha-quickslot',passed:checks.length-failed.length,failed:failed.length,checks};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82IL2_MOROC_QUICKSLOT.json'),JSON.stringify(report,null,2)+'\n');
