...
assert.strictEqual(core.version,'0.9.82BA');
assert.strictEqual(Object.keys(core.skills).length,568);
assert.strictEqual(generated.summary.officialRuntime,568);
assert.strictEqual(generated.summary.pending,571);
assert.strictEqual(pending.skills.length,571);
for(const id of ids){
...
for(const id of [10,93,1006]){
  assert.strictEqual(core.skills[String(id)].handler,'passive');
  assert.deepStrictEqual(core.skills[String(id)].passiveBonuses,{atkRate:5,matkRate:5});
}
...
// Skill-engine behavior kept from Batch56.
{
 const c=baseContext();
...
// Energy Coat behavior kept from Batch56.
{
 const c=baseContext();
...
console.log('Mage / High Wizard foundation regression 0.9.82BA: PASS');
