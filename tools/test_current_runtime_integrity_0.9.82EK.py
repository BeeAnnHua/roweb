...
COMMANDS=[
 ...
 ['node','tools/test_status_advanced_ui_0.9.82EK.js'],
 ['node','tools/test_status_responsive_ui_0.9.82EK.js'],
 ['node','tools/test_roitemsearch_modifier_bridge_0.9.82EK.js'],
 ['python','tools/test_region_monster_streaming_0.9.82EK.py'],
 ['node','tools/test_ra_monster_ai_spawn_chase_0.9.82EK.js'],
 ['node','tools/test_monster_hp_manual_click_0.9.82EK.js'],
 ...
 ['python','tools/build_data_bundle_0.9.82EK.py'],
 ['python','tools/deep_health_check.py','.','--json-output','tools/deep_health_report_0.9.82EK.json'],
]
rows=[]
for cmd in COMMANDS:
 ...
 if cp.returncode:
  result={'version':'0.9.82EK','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82EK.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82EK','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82EK.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
