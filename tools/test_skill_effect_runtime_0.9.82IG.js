...
(async()=>{
  ...
  checks.push(['acidified_ground_uses_target_payload',ground.every(x=>x.fixedAnchor?.worldPosition?.x===900&&x.fixedAnchor?.worldPosition?.y===640)]);
  ...
  checks.push(['flushed_events_use_hit_target',flushed.length>=6&&flushed.every(x=>x.fixedAnchor?.worldPosition?.x===1110&&x.fixedAnchor?.worldPosition?.y===720)]);
  ...
  checks.push(['self_test_anchor_contract',api.selfTest().anchorPolicy==='ACIDIFIED_ALL_PHASE_TARGET_GROUND_SNAPSHOT_CASTER_BUFFS_LIVE_TARGET_HITS_LIVE']);
