...
check('RO_WEB V0.9.82FY' in index,'index version')
check('const RO_WEB_VERSION = "0.9.82FY"' in game,'game version')
cache_versions=set(re.findall(r'\?v=([^"\']+)',index))
check(cache_versions=={'0.9.82FY'},'cache version consistency',str(sorted(cache_versions)))
...
check('js/card_runtime.js?v=0.9.82FY' in index and 'js/effect_runtime.js?v=0.9.82FY' in index,'FX unified Runtime scripts loaded')
check(index.index('js/card_runtime.js?v=0.9.82FY') < index.index('js/effect_runtime.js?v=0.9.82FY') < index.index('js/status_system.js?v=0.9.82FY'),'FX Runtime load order')
check('js/effect_visual_runtime.js?v=0.9.82FY' in index,'FX transform visual Runtime loaded')
...
report={
    'version':'0.9.82FY','checks':len(checks),'passed':sum(1 for _,ok,_ in checks if ok),
...
(ROOT/'tools/test_report_0.9.82FY.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
