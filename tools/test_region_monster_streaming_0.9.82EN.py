...
check(config.get('version')=='0.9.82EN','spawn config version')
check(manifest.get('version')=='0.9.82EN','manifest version')
...
check(ai_modes.get('version')=='0.9.82EN','AI mode version')
...
check(versions=={'0.9.82EN'},f'cache versions {versions}')
check('RO_WEB V0.9.82EN' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EN','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
