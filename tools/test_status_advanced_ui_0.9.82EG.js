...
for(const token of ['status-allocation-step-button','status-advanced-toggle','status-advanced-panel','status-advanced-inline','advanced-inline-mode','傷害','詠唱／延遲','生存／耐性','體型傷害','種族傷害','種族別暴擊率','種族別防禦穿透','屬性別防禦穿透','階級別防禦穿透','敵人屬性別傷害減免','武器體型懲罰','固定詠唱減免','變動詠唱－綜合估算']) assert(statusSource.includes(token),`Missing UI token ${token}`);
...
assert(!statusSource.includes('角色＋裝備＋卡片＋技能＋Buff 最終加總'),'Advanced subtitle removed');
assert(!statusSource.includes('依 rAthena Renewal 與 ROItemSearch 效果分類顯示'),'Advanced intro removed');
...
eq(policy.version,'0.9.82EG','Policy version');
...
eq(bridge.version,'0.9.82EG','ROItemSearch bridge version');
...
assert((html.match(/\?v=0\.9\.82EG/g)||[]).length>=30,'All entry assets use EG cache key');

console.log('PASS 0.9.82EG cache-safe global +10 and ROItemSearch-expanded combat summary');
