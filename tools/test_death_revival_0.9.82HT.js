...
check(html.includes('death_revival_runtime.js?v=0.9.82HT'), '死亡 Runtime 以 HT 快取版本載入');
...
function element() { return { hidden:true, disabled:false, textContent:'', title:'', classList:classList(), setAttribute(){}, focus(){this.focused=true} }; }
const elements = {
...
const report = {
  version:'0.9.82HT', suite:'death-revival-token-auto-revive-gacha',
  passed:passes.length, failed:failures.length, passes, failures
};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HT_DEATH_REVIVAL.json'),JSON.stringify(report,null,2)+'\n');
