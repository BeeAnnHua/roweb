...
for(const token of ['RO_WEB_DAMAGE_NUMBER_BATCH','createDocumentFragment','requestAnimationFrame','scheduleDamageNumberBatch','maxPerFrame: 24'])assert(battle.includes(token),`missing damage batch token ${token}`);
...
console.log(JSON.stringify({version:'0.9.82ER',status:'PASS',damageNumbers:'animation-frame batch',combatSave:'300ms debounce',castStats:'target-scoped evaluation context'},null,2));
