...
check('version_title','RO_WEB V0.9.83B' in index)
versions=set(re.findall(r'[?&]v=([^&"\']+)',index))
check('cache_bust_only_v083',versions=={'0.9.83B'},sorted(versions))
...
result={'version':'0.9.83B','baseVersion':'0.9.82IL4','pass':not errors,'checks':checks,'errors':errors,'summary':{'passed':sum(v['pass'] for v in checks.values()),'total':len(checks),'jsSyntax':f'{syntax_ok}/{len(jsfiles)}'}}
