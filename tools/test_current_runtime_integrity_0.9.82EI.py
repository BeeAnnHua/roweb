...
COMMANDS=[
 ...
 ['node','tools/test_status_advanced_ui_0.9.82EI.js'],
 ['node','tools/test_status_responsive_ui_0.9.82EI.js'],
 ['node','tools/test_roitemsearch_modifier_bridge_0.9.82EI.js'],
 ['python','tools/test_region_monster_streaming_0.9.82EI.py'],
 ...
 ['python','tools/build_data_bundle_0.9.82EI.py'],
 ['python','tools/deep_health_check.py','.','--json-output','tools/deep_health_report_0.9.82EI.json'],
]
rows=[]
for cmd in COMMANDS:
 ...
 if cp.returncode:
  result={'version':'0.9.82EI','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82EI.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82EI','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82EI.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
