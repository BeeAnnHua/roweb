...
check('index uses HU cache version', '0.9.82HP' not in html_source and '?v=0.9.82HU' in html_source)
...
with sync_playwright() as p:
    ...
    page.screenshot(path=str(out/'CURRENCY_INLINE_EXPAND_0.9.82HU_desktop.png'),full_page=False)
    ...
    mobile.screenshot(path=str(out/'CURRENCY_INLINE_EXPAND_0.9.82HU_mobile.png'),full_page=False)
    browser.close()

check('no browser runtime errors',not errors,errors)
failed=[x for x in checks if not x['pass']]
report={'version':'0.9.82HU','passed':len(checks)-len(failed),'failed':len(failed),'checks':checks,'errors':errors}
(ROOT/'tools/test_hu_currency_inline_expand_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
