...
check('RO_WEB V0.9.82GA' in index,'index version')
check('const RO_WEB_VERSION = "0.9.82GA"' in game,'game version')
check(set(re.findall(r'\?v=([^"\']+)',index))=={'0.9.82GA'},'cache version consistency')
check('js/mvp_gacha_runtime.js?v=0.9.82GA' in index,'MVP gacha Runtime loaded')
check(index.index('js/item_instance_ui.js?v=0.9.82GA') < index.index('js/mvp_gacha_runtime.js?v=0.9.82GA') < index.index('js/loot.js?v=0.9.82GA'),'gacha Runtime load order')
...
audit=load('tools/melee_family_targeting_audit_0.9.82GA.json')
check(audit.get('status')=='PASS' and not audit.get('issues'),'five melee-family targeting audit passes',audit.get('issues'))

report={'version':'0.9.82GA','summary':{'checks':len(checks),'passed':sum(x['ok'] for x in checks),'failed':len(errors)},'errors':errors,'checks':checks}
(ROOT/'tools/test_report_0.9.82GA.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
