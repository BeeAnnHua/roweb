...
COMMANDS=[
 ...
 ['node','tools/test_formula_authority_map_cleanup_0.9.82EF.js'],
 ...
 ['node','tools/test_status_advanced_ui_0.9.82EF.js'],
 ['node','tools/test_roitemsearch_modifier_bridge_0.9.82EF.js'],
 ...
 ['python','tools/build_data_bundle_0.9.82EF.py'],
 ['python','tools/deep_health_check.py','.','--json-output','tools/deep_health_report_0.9.82EF.json'],
]
rows=[]
for cmd in COMMANDS:
 ...
 if cp.returncode:
  result={'version':'0.9.82EF','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82EF.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82EF','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82EF.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
