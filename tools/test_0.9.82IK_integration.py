...
check('version_title','RO_WEB 0.9.82IK' in index)
versions=set(re.findall(r'[?&]v=([^&"\']+)',index))
check('cache_bust_only_ik',versions=={'0.9.82IK'},sorted(versions))
...
result={'version':'0.9.82IK','baseVersion':'0.9.82IJ','pass':not errors,'checks':checks,'errors':errors,'summary':{'passed':sum(v['pass'] for v in checks.values()),'total':len(checks),'jsSyntax':f'{syntax_ok}/{len(jsfiles)}'}}
(ROOT/'TEST_REPORT_0.9.82IK_INTEGRATION.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
