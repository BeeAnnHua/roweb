...
assert config['version']=='0.9.82HQ',config.get('version')
...
assert bundled['version']=='0.9.82HQ',bundled.get('version')
assert bundled['server']['rates']==expected,bundled['server']['rates']
print(json.dumps({'version':'0.9.82HQ','passed':2,'failed':0,'rates':expected},ensure_ascii=False,indent=2))
