...
full_skill=load('tools/full_skill_audit_0.9.82DZ.json')
ra=load('tools/ra_skill_timing_audit_0.9.82DZ.json')
action=load('tools/all_skill_timing_action_audit_0.9.82DZ.json')
combat=load('tools/ra_renewal_combat_audit_0.9.82DZ.json')
critical=load('tools/ra_renewal_skill_critical_flags_audit_0.9.82DZ.json')
range_audit=load('tools/ra_skill_range_audit_0.9.82DZ.json')
distance=load('tools/all_skill_distance_profile_audit_0.9.82DZ.json')
pending_scope=load('tools/pending_skill_scope_audit_0.9.82DZ.json')
legacy_cleanup=load('tools/legacy_formula_cleanup_audit_0.9.82DZ.json')
open_work=load('docs/OPEN_WORK_ITEMS_0.9.82DZ.json')
...
result={
 ...
 'knownLimitations':[
   ...
   'Ground-spell wave/tick/overlap/legal-cell behavior still requires the dedicated next-stage audit, beginning with Storm Gust.',
   'Historical regression scripts that hard-code older timing assumptions are not authoritative for the DZ Renewal layer; dedicated 0.9.82DZ tests are authoritative.',
...
out=ROOT/'docs/FULL_SKILL_CHARACTER_RUNTIME_AUDIT_0.9.82DZ.json'
