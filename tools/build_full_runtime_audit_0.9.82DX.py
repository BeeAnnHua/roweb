...
full_skill=load('tools/full_skill_audit_0.9.82DX.json')
ra=load('tools/ra_skill_timing_audit_0.9.82DX.json')
action=load('tools/all_skill_timing_action_audit_0.9.82DX.json')
combat=load('tools/ra_renewal_combat_audit_0.9.82DX.json')
critical=load('tools/ra_renewal_skill_critical_flags_audit_0.9.82DX.json')
...
result={
 ...
 'knownLimitations':[
   ...
   'Historical regression scripts that hard-code older timing assumptions are not authoritative for the DX Renewal timing layer; dedicated 0.9.82DX tests are authoritative.',
 ...
 'summary':{
   'status':'PASS' if not errors and not full_skill['errors'] and not ra['errors'] and not action['errors'] and combat['summary']['status']=='PASS' and not critical['errors'] else 'FAIL',
   'skillErrors':len(full_skill['errors'])+len(ra['errors'])+len(action['errors'])+len(critical['errors']),
   'characterErrors':len(errors),
   'warnings':len(full_skill['warnings'])+len(warnings)
 }
}
out=ROOT/'docs/FULL_SKILL_CHARACTER_RUNTIME_AUDIT_0.9.82DX.json'
