...
check('RO_WEB V0.9.82FV' in index,'index version')
check('const RO_WEB_VERSION = "0.9.82FV"' in game,'game version')
cache_versions=set(re.findall(r'\?v=([^"\']+)',index))
check(cache_versions=={'0.9.82FV'},'cache version consistency',str(sorted(cache_versions)))
...
report={
    'version':'0.9.82FV','checks':len(checks),'passed':sum(1 for _,ok,_ in checks if ok),
...
(ROOT/'tools/test_report_0.9.82FV.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
