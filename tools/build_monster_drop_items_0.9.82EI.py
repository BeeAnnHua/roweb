...
ICON_ZIP=Path('/mnt/data/RO_WEB專案資料.zip')
OUT=ROOT/'data/items/monster_drops_0_9_82EI.json'
...
def load_existing_ids():
    ...
    for rel in manifest.get('allDataPaths',[]):
        p=ROOT/rel
        if not p.exists() or p.name=='monster_drops_0_9_82EI.json': continue
...
def drop_ids():
    ...
    return {int(d['itemId']) for m in mons for d in m.get('drops',[]) if int(d.get('itemId',0))>0}
...
def make_record(iid,ra,info):
    ...
    rec={
      ...
      'dataSource':'itemInfo_UTF8.lub identified fields + rAthena Renewal db/re item DB; activated for 0.9.82EI regional monster drops',
...
def extract_icons(ids:set[int]):
    ...
    with zipfile.ZipFile(ICON_ZIP) as z:
        suffix_to_name={}
        for n in z.namelist():
            if '/items/' in n and n.endswith('.webp'):
                suffix=n.rsplit('/items/',1)[1]
...
def main():
    ...
    manifest['monsterDropActivation']={
      'version':VERSION,'recordCount':len(records),'source':'111 RA regional monster records',
      'note':'Only item IDs referenced by the active regional monster pools and missing from the existing split Item DB are added here.'
    }
    ...
    (ROOT/'tools/monster_drop_item_audit_0.9.82EI.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
