...
check(cfg.get("version") == "0.9.83A", "config version")
...
check(cfg.get("binding") == {"characterBound": True, "noStorage": True, "noDecompose": False, "noSell": False}, "binding policy")
...
for item_id in all_equipment_ids:
    ...
    check(row.get("noDecompose") is False, f"decompose allowed {item_id}")
    check(row.get("noSell") is False, f"sell allowed {item_id}")
    check(isinstance(row.get("sellPrice"), (int, float)) and row.get("sellPrice") > 0, f"positive sell price {item_id}")
...
check("const VERSION = '0.9.83A'" in runtime, "runtime version")
check('RO_WEB_SAVE_APP_VERSION = "0.9.83A"' in player_js, "save version")
...
check("item-detail-sell-action" in index_html, "support sell button present")
check("sellSupportEquipmentInstance" in item_ui and "configureSupportEquipmentSellAction" in item_ui, "support sell runtime present")
check("noStorage" in storage_js and "新人支援裝備不可存入帳號共用倉庫" in storage_js, "storage rejection")
check("newcomer_support_runtime.js?v=0.9.83A" in index_html, "runtime script loaded")
check("RO_WEB V0.9.83A" in index_html, "page title version")
...
check('"version":"0.9.83A"' in bundle or "version: '0.9.83A'" in bundle or 'version:"0.9.83A"' in bundle, "data bundle version")
...
result = {"version": "0.9.83A", "checks": checks, "passed": checks - len(failures), "failed": len(failures), "failures": failures}
