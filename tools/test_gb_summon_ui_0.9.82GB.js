...
(function(){
  ...
  check(quick.includes('三轉後不再使用二轉騎乘術')&&quick.includes('openVirtualSummonWindow'),'Third-job Riding redirects to summon control instead of toggling Dragon');
  const manifest=read('data/mounts/mount_manifest.json');
  check(manifest.mounts.dragon.control==='virtual_summon_window'&&manifest.mounts.dragon.toggleSkillId===null,'Dragon mount is controlled by summon UI',JSON.stringify(manifest.mounts.dragon));
...
fs.writeFileSync(path.join(ROOT,'tools/test_gb_summon_ui_report_0.9.82GB.json'),JSON.stringify(report,null,2)+'\n');
