...
assert(battle.includes('updateWorldMonsterFieldTestUi(monsterSnapshot)'), 'Immediate monster HP refresh missing');
const css = fs.readFileSync('css/style.css', 'utf8');
assert(css.includes('transition: width .035s linear'), 'Fast HP width response missing');
const html = fs.readFileSync('index.html', 'utf8');
assert((html.match(/\?v=0\.9\.82EM/g) || []).length >= 30, 'EM cache keys incomplete');

console.log('PASS 0.9.82EM monster name, movement animation, instant HP and RA plant damage');
console.log(JSON.stringify({ monsters:111, renamed:44, infiniteDefensePlants:8, fixedDamagePerHit:1, fiveHitDamage:5, cache:'0.9.82EM' }, null, 2));
