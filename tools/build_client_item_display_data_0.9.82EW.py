...
def resolve_iteminfo():
    candidates = []
    if len(sys.argv) > 1:
        candidates.append(Path(sys.argv[1]))
    if os.environ.get('RO_WEB_ITEMINFO'):
        candidates.append(Path(os.environ['RO_WEB_ITEMINFO']))
    candidates.extend([
        ROOT / 'itemInfo_UTF8.lub',
        ROOT / 'client_tables' / 'itemInfo_UTF8.lub',
        Path('/mnt/data/itemInfo_UTF8.lub'),
    ])
    ...
    raise FileNotFoundError(
        '找不到 itemInfo_UTF8.lub。可將檔案放在專案根目錄、client_tables，'
        '或以命令列第一個參數／RO_WEB_ITEMINFO 環境變數指定。\n' + searched
    )

ITEMINFO = resolve_iteminfo()
