...
def main():
    ...
    try:
        ...
        # V92 skill-effect deployment JSON is intentionally fetched as a separate
        # browser-safe library and must not be duplicated into the monolithic data bundle.
        runtime_json=[p for p in runtime_json if not str(p.relative_to(root)).replace("\\","/").startswith("assets/skill_effects/v92/")]
