...
check(config.get('version')=='0.9.82EW','spawn config version')
check(manifest.get('version')=='0.9.82EW','manifest version')
...
check(ai_modes.get('version')=='0.9.82EW','AI mode version')
...
check('queueMonsterDefeatResolution(defeatedMonster, { primary:true })' in battle,'deferred battle death hook missing')
...
check(versions=={'0.9.82EW'},f'cache versions {versions}')
check('RO_WEB V0.9.82EW' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EW','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
