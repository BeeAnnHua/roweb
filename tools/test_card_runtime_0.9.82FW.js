...
const report={version:'0.9.82FW',cards:Object.keys(effects).length,combos:combos.length,dropSources:Object.values(drops).reduce((n,x)=>n+x.length,0),runtimeErrors:runtimeErrors.length,unknownBonuses:raw,socketRemovalTests:failures.length? 'FAIL':'PASS',failures};
fs.writeFileSync(path.join(ROOT,'tools/test_card_runtime_report_0.9.82FW.json'),JSON.stringify(report,null,2)+'\n');
