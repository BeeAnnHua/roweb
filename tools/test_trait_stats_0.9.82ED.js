...
assertEq(context.allocateTraitPoints('pow', 10), 10, 'Allocate POW +10');
assertEq(context.player.traits.pow, 11, 'POW +10 stored');
...
for (const token of ['status-trait-toggle','status-trait-panel','status-trait-plus-ten','getTraitBreakdownTooltip','四轉 Job 加成','裝備／卡片','被動技能','目前 Buff','免費重置特性','Base Lv.200 轉為四轉後即可獲得 7 點']) {
  assertTrue(statusSource.includes(token), `UI source includes ${token}`);
}
const styleSource = fs.readFileSync('css/style.css', 'utf8');
for (const token of ['.status-trait-toggle','.status-trait-panel','.status-trait-plus','.status-trait-plus-ten','.status-trait-reset']) {
...
console.log('PASS 0.9.82ED rAthena Renewal trait stats, Lv200 fourth-job bonus, doubled level points, allocation and critical formula');
