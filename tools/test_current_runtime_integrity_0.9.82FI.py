...
COMMANDS=[
 ...
 ['node','tools/test_release_cleanup_town_restore_map_tooltip_0.9.82FI.js'],
 ['node','tools/test_ui_window_quickslot_layout_0.9.82FI.js'],
 ['node','tools/test_ui_accessory_damage_movement_recovery_0.9.82FI.js'],
 ['node','tools/test_default_prontera_start_0.9.82FI.js'],
 ...
 ['node','tools/test_auto_battle_attack_heading_0.9.82FI.js'],
 ['node','tools/test_map_spawn_smoothing_0.9.82FI.js'],
 ...
 ['node','tools/test_map_quest_cast_buff_0.9.82FI.js'],
...
for idx,cmd in enumerate(COMMANDS,1):
 ...
 if cp.returncode:
  result={'version':'0.9.82FI','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82FI.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82FI','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82FI.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
