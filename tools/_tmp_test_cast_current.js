...
const ctx={console,Math:Object.create(Math),Date,JSON,Number,String,Object,Array,Set,Map,Promise,performance:{now:()=>Date.now()},window:{},document:{readyState:'complete',getElementById:()=>null,addEventListener:()=>{},body:{appendChild:()=>{}},createElement:()=>({style:{},classList:{add:()=>{},remove:()=>{},toggle:()=>{}},setAttribute:()=>{},appendChild:()=>{},querySelector:()=>null})},
...
// rAthena Renewal applies unit_set_attackdelay at every player skill cast begin.
// At ASPD 193 this 140ms action lock is longer than min_skill_delay_limit=100ms.
...
assert(actionDelta>=135&&actionDelta<=165,'instant magic receives the RA 140ms cast-begin action lock');
let actionBlock=ctx.getRuntimeSkillDelayBlock(instantPhysical,1);
assert(actionBlock&&actionBlock.type==='aspd','RA cast-begin action lock blocks every active skill');
...
// Cart Cannon Lv5: 3000ms variable cast, 500ms AfterCastActDelay, no cooldown.
// Zero variable cast + 75% delay reduction leaves 125ms, but the exact RA
// ASPD-193 cast-begin lock is 140ms, yielding about 7.14 casts/second without
// any artificial network/server cap. 83% leaves 85ms and remains RA-lock limited.
const cartCannon={id:2477,officialId:2477,type:'Weapon',castTime:[{Level:5,Time:3000}],afterCastActDelay:500};
ctx.skillsData.runtimeProfiles['2477']={handler:'physical_attack_formula',formula:'renewal_cart_cannon'};
ctx.player.aspd=193;ctx.player.stats.dex=200;ctx.player.stats.int=130;ctx.player.equipment={weapon:1};
active={afterCastDelayReductionRate:75};
let cartTiming=ctx.getRuntimeSkillTimingProfile(cartCannon,5);
assert(cartTiming.cast.totalMs===0,'Cart Cannon reaches zero cast at Renewal stat threshold');
assert(cartTiming.afterCastActDelayMs===125,'Cart Cannon 75% after-cast reduction');
assert(ctx.getRuntimeSkillCastBeginLockMs(cartCannon,5)===140,'Cart Cannon exact RA ASPD-193 action lock');
assert(Math.max(cartTiming.afterCastActDelayMs,ctx.getRuntimeSkillCastBeginLockMs(cartCannon,5))===140,'Cart Cannon reaches natural RA sec-7 interval');
active={afterCastDelayReductionRate:83};
cartTiming=ctx.getRuntimeSkillTimingProfile(cartCannon,5);
assert(cartTiming.afterCastActDelayMs===85,'Cart Cannon 83% after-cast reduction');
assert(Math.max(cartTiming.afterCastActDelayMs,ctx.getRuntimeSkillCastBeginLockMs(cartCannon,5))===140,'83% remains limited only by exact RA action lock');
active={};
...
console.log('PASS 0.9.82EA Renewal timing/combat tests');
