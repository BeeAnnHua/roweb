...
check(R.version==='0.9.82HR'||R.version==='0.9.82HX','global rare runtime version');
...
// Gacha uses per-item absolute probability for special rewards.
...
check(anniversary===10&&R.tierForChanceBasisPoints(anniversary)==='purple','20th anniversary rewards are 0.1% each',String(anniversary));
check(temporal===100&&R.tierForChanceBasisPoints(temporal)==='red','Temporal Transcendent rewards are 1% each',String(temporal));
const ordinaryTotalWeight=cfg.ordinaryRewards.reduce((sum,row)=>sum+Number(row.weight||0),0);
const ordinaryDuplicateWeight=cfg.ordinaryRewards.filter(row=>Number(row.itemId)===Number(ordinary79.itemId)).reduce((sum,row)=>sum+Number(row.weight||0),0);
const ordinaryExpected=cfg.ordinaryFillBasisPoints*ordinaryDuplicateWeight/ordinaryTotalWeight;
check(Math.abs(ordinaryChance-ordinaryExpected)<1e-9,'ordinary weighted probability computed from real pool',String(ordinaryChance));
// The new 5% token-box category reduces only the ordinary fill; duplicate rows must still aggregate before tiering.
check(ordinaryChance>100&&R.tierForChanceBasisPoints(ordinaryChance)===null,'duplicate ordinary reward is aggregated before tiering, no false rare banner',String(ordinaryChance));
...
const report={version:'0.9.82HX',summary:{checks:results.length,passed:pass,failed:fail,status:fail?'FAIL':'PASS'},results};
fs.writeFileSync(path.join(ROOT,'TEST_REPORT_0.9.82HX_RARE_ITEM_ANNOUNCEMENT.json'),JSON.stringify(report,null,2)+'\n');
console.log(`\nRare announcement HT: ${pass} passed, ${fail} failed`);
