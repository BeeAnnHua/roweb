...
for cat in gacha.get('rareCategories',[]):
    rewards=cat.get('rewards',[])
    if str(cat.get('chanceMode','')) == 'per_reward_absolute':
        agg=defaultdict(float)
        for row in rewards: agg[str(row.get('itemId'))]+=max(0,float(row.get('chanceBasisPoints',0)))
        for iid,bp in agg.items():
            gacha_rows.append({'pool':cat.get('id'),'itemId':int(iid),'chanceBasisPoints':bp,'chancePercent':bp/100,'tier':tier(bp)})
    else:
        total=sum(max(0,float(r.get('weight',0))) for r in rewards)
        agg=defaultdict(float)
        for row in rewards: agg[str(row.get('itemId'))]+=max(0,float(row.get('weight',0)))
        for iid,w in agg.items():
...
(ROOT/'RARE_ITEM_PROBABILITY_AUDIT_0.9.82HW.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
lines=['# RO_WEB 0.9.82HW 稀有物品機率稽核','', '- 門檻：≤1% 紅、≤0.1% 紫、≤0.01% 金。','- 加權池內相同 Item ID 會合併權重後再判定，避免同物品重複列造成假稀有。','- 升階材料固定 5%，不觸發稀有公告。','']
...
lines.append("- MVP 轉蛋特殊獎：基爾卡 0.01%；20週年兩件各 0.1%；時光超越者五件各 1%；LT 四件各 0.1%；復活證箱 5%。")
lines.append("- 特殊獎合計 10.61%；普通獎池 89.39%。")
lines.append(f"- 升階材料設定列：{len(grade_rows)} 列，全部 5%。")
(ROOT/'RARE_ITEM_PROBABILITY_AUDIT_0.9.82HW.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
print(json.dumps({'version':'0.9.82HW','boxes':{k:v['tierCounts'] for k,v in box_report.items()},'gacha':report['gacha']['tierCounts'],'gradeEntries':len(grade_rows),'gradeAll5Percent':report['gradeMaterials']['allExactlyFivePercent']},ensure_ascii=False,indent=2))
