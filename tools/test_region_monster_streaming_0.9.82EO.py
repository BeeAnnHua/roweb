...
check(config.get('version')=='0.9.82EO','spawn config version')
check(manifest.get('version')=='0.9.82EO','manifest version')
...
check(ai_modes.get('version')=='0.9.82EO','AI mode version')
...
check(versions=={'0.9.82EO'},f'cache versions {versions}')
check('RO_WEB V0.9.82EO' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EO','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
