...
COMMANDS=[
 ...
 ['node','tools/test_auto_battle_attack_heading_0.9.82FC.js'],
 ...
 ['node','tools/test_map_quest_cast_buff_0.9.82FC.js'],
...
for cmd in COMMANDS:
 ...
 if cp.returncode:
  result={'version':'0.9.82FC','status':'FAIL','testCount':len(rows),'tests':rows}
  (ROOT/'tools/current_runtime_integrity_0.9.82FC.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
  print(json.dumps(result,ensure_ascii=False,indent=2));sys.exit(cp.returncode)
result={'version':'0.9.82FC','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82FC.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
