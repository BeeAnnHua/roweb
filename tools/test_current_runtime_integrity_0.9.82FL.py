...
COMMANDS = [
    ...
    ['node', 'tools/test_auto_battle_monster_filter_0.9.82FL.js'],
    ['node', 'tools/test_aspd_attack_visual_ui_size_0.9.82FL.js'],
    ['node', 'tools/test_auto_battle_attack_heading_0.9.82FL.js'],
...
for idx, cmd in enumerate(COMMANDS, 1):
    ...
    try:
        cp = subprocess.run(cmd, cwd=ROOT, capture_output=True, text=True, timeout=240)
    except subprocess.TimeoutExpired as exc:
        cp = subprocess.CompletedProcess(cmd, 124, stdout=exc.stdout or '', stderr=(exc.stderr or '')+'\nTIMEOUT')
    rows.append({'command':' '.join(cmd),'status':'PASS' if cp.returncode == 0 else 'FAIL','returnCode':cp.returncode,'tail':(cp.stdout+cp.stderr).strip()[-2400:]})
    if cp.returncode:
        result={'version':'0.9.82FL','status':'FAIL','testCount':len(rows),'tests':rows}
        (ROOT/'tools/current_runtime_integrity_0.9.82FL.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
        print(json.dumps(result,ensure_ascii=False,indent=2)); sys.exit(cp.returncode)
result={'version':'0.9.82FL','status':'PASS','testCount':len(rows),'tests':rows}
(ROOT/'tools/current_runtime_integrity_0.9.82FL.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
