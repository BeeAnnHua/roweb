...
check('RO_WEB_SAVE_APP_VERSION = "0.9.83B"' in player_js, "save version")
...
check("newcomer_support_runtime.js?v=0.9.83B" in index_html, "runtime script loaded")
check("RO_WEB V0.9.83B" in index_html, "page title version")
...
result = {"version": "0.9.83B", "checks": checks, "passed": checks - len(failures), "failed": len(failures), "failures": failures}
