...
def main():
    ...
    index_text = (root / "index.html").read_text(encoding="utf-8")
    rare_tag = './js/rare_item_announcement_runtime.js?v=0.9.82HS'
    if rare_tag not in index_text: a.error("HR_RARE_RUNTIME_INDEX", rare_tag)
    else:
        try:
            rare_pos=index_text.index(rare_tag); gacha_pos=index_text.index('./js/mvp_gacha_runtime.js?v=0.9.82HS'); box_pos=index_text.index('./js/item_box_runtime.js?v=0.9.82HS')
    ...
    # 0.9.82HS manual combat target authority and aggressive-monster retaliation.
    ...
    hs_report = root / "TEST_REPORT_0.9.82HS_MANUAL_COMBAT.json"
