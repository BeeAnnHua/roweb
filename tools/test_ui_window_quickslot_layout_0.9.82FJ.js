...
assert(ui.includes('CSS.supports("zoom", "0.75")') && ui.includes('`scale(${effectiveFactor})`'),'Zoom/transform cross-browser fallback missing');
assert(ui.includes('button.addEventListener("touchend", activateSizeCycle'),'Mobile size switch missing');
assert(ui.includes('target.style.setProperty("zoom", String(effectiveFactor), "important")'),'Inline important zoom override missing');
assert(ui.includes('const baseFactor = getWindowBaseVisualScale(target);'),'Legacy inventory/skill base scale missing');
assert(css.includes('#inventory-window.ui-size-enabled { --ro-ui-base-zoom: .92; }'),'Inventory base zoom contract missing');
assert(css.includes('#skill-window.true-skill-window.ui-size-enabled { --ro-ui-base-zoom: .92; }'),'Skill base zoom contract missing');
...
assert(game.includes('const RO_WEB_VERSION = "0.9.82FJ";'),'Runtime version must be FI');
assert([...index.matchAll(/\?v=([^"']+)/g)].every(m=>m[1]==='0.9.82FJ'),'All cache keys must be FI');
console.log(JSON.stringify({version:'0.9.82FJ',status:'PASS',windowSizes:['100%','75%','50%'],mobileTouchCycle:true,offscreenRecovery:true},null,2));
