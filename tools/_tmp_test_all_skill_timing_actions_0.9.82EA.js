...
const ctx={console,Math,Date,JSON,Number,String,Object,Array,Set,Map,Promise,performance:{now:()=>Date.now()},window:{},document:{readyState:'complete',getElementById:()=>null,addEventListener:()=>{},body:{appendChild:()=>{}},createElement:()=>({style:{},classList:{add:()=>{},remove:()=>{},toggle:()=>{}},setAttribute:()=>{},appendChild:()=>{},querySelector:()=>null})},
...
for(const [id,row] of Object.entries(runtime)){
  ...
  if(handler==='passive'){
  ...
  }else if(handler && handler!=='pending' && ctx.getRuntimeSkillUiType(s)==='attack'){
    counts.attack++; if(motion!=='attack')err(`${id}: damage motion ${motion}`);
    if(ctx.isRuntimePhysicalAttackSkill(s,rp)){
    ...
    counts.cast++; if(motion!=='cast')err(`${id}: non-damage active motion ${motion}`);
  }
  for(let lv=1;lv<=max;lv++){
    ...
    if(handler && handler!=='pending' && handler!=='passive'){
      ...
      const performanceFloor=ctx.getRuntimeSkillUiType(s)==="attack"?Number(ctx.RO_WEB_ACTIVE_ATTACK_MIN_INTERVAL_MS||140):0;
      const expected=Math.max(Number(t.minSkillDelayLimitMs||100),Number(t.cast.totalMs||0),Number(ctx.getPlayerSkillActionLockMs()||0),performanceFloor);
      ...
      if(!["aspd","cast","throughput","action_lock"].includes(String(state.actionLockType||"")))err(`${id} Lv${lv}: cast-begin lock type ${state.actionLockType}`);
...
const result={version:'0.9.82EA',renewalOnly:true,runtimeSkills:Object.keys(runtime).length,counts,errors,summary:{status:errors.length?'FAIL':'PASS',errors:errors.length}};
fs.writeFileSync(path.join(ROOT,'tools/all_skill_timing_action_audit_0.9.82EA.json'),JSON.stringify(result,null,2)+'\n');
