...
check(config.get('version')=='0.9.82ES','spawn config version')
check(manifest.get('version')=='0.9.82ES','manifest version')
...
check(ai_modes.get('version')=='0.9.82ES','AI mode version')
...
check(versions=={'0.9.82ES'},f'cache versions {versions}')
check('RO_WEB V0.9.82ES' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82ES','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
