# 0.9.82FM CURRENT RUNTIME
目前正式版本：**0.9.82FM**。

多段傷害顯示使用同一次結算的總傷害與 `hitCount` 建立累積視覺序列；不得把總傷害重新計算成多次實際傷害，也不得重複套用防禦、屬性、暴擊或吸血。視覺最多建立 30 段，最後一段必須顯示精確總傷害。普通攻擊的二刀／六合拳、技能多段、獵鷹與其他追加來源都沿用同一 `showDamageNumber(..., { hitCount })` 管線；追加來源維持黃色樣式。

自動戰鬥近戰追擊只對移動中的怪物、近戰距離及自動模式啟用有限預測位置與 24px 動態距離補償，不得改變玩家手動攻擊、遠距離技能或正式技能射程。任何瞬移完成後都必須清除 controller target、`currentTarget`、選取框與舊移動路徑，短暫抑制重新選怪，並在約 3 秒內排除瞬移前的舊怪物。

世界地圖共有 11 個獨立 3×3 地區。`thanatos_tower_3x3_region_camera` 使用來源圖塊 091～099、4608×4608 世界尺寸，怪物池為 21 種；能力、AI 與掉落來源為 rAthena Renewal。1704～1707 為獨立稀有怪，1708「魔劍士 達納托斯的記憶」為獨立 MVP，不納入普通怪物倍率。

城鎮共用補品商店新增 505、11621～11624、1100003～1100005。消耗品 Runtime 必須解析等級需求與物品共用延遲；隨機 `itemheal` 不可在資料匯入時固定成單一值。手動使用與自動補品共用 `canUseConsumableItem()`／`markConsumableItemUsed()`。

自動戰鬥面板的 body 必須在可視高度內獨立捲動，手機使用 `touch-action: pan-y`，底部 Buff 與儲存按鈕不可被視窗高度或舊 overflow 規則裁切。FL 的怪物指定／排除篩選、FK 的自動肯貝特與地圖傳送捲動契約全部保留。
...
本版以 0.9.82FI 為基準，修正 ASPD 高速普通攻擊的動畫／特效同步，以及背包、技能欄在桌機與手機的大中小尺寸切換。
