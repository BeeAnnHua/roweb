...
- 以 `roweb(20260817-032930).zip` 為最新版母檔；B10 低流量前端與既有 SQL 契約完整保留，本版不需再執行 SQL。
- 修正 10 份 Generic12 特效 JSON 中的非法 `Infinity`，並在 Runtime 對非有限動畫增量加上 0 值兜底。
- localStorage 主存檔維持即時；重複的安全備份副本改為每 60 秒最多一次，關鍵存檔仍強制更新。
- `pagehide`／`beforeunload`／`freeze` 兩秒內去重，避免關閉一次頁面連續建立多份完整快照。
- 光耀之堂延遲 Tick 固定真正施法者，傭兵施放時不再把主角誤當治療來源或技能中心。
- 新增 B11 當前版健康測試並統一玩家端版本／快取標記。

## V0.9.88B9

- 傭兵攻擊／輔助模式新增「技能設定」視窗；每名傭兵只顯示該分身已學會且列入傭兵目錄的技能，可分別勾選並持久保存。
- 攻擊技能目錄 197 招；輔助技能目錄 99 招（群體 69、單體／復活 30）；光之障壁（25）依決議排除。
- 傭兵技能直接使用玩家原 Skill Engine 的公式、SP、詠唱、冷卻、命中、多段、狀態與特效；自創技能及既有 600 多招資料完全保留。
- 攻擊型傭兵遇到 SP 不足、冷卻中、條件不符或未勾選可用技能時，自動退回普通攻擊。
- 治療術會挑選低血量友軍；痊癒術及 2047／2048 群體解除會先偵測有效異常；復活術（54）可主動救起倒下的玩家或傭兵。
- 傭兵施放期間使用獨立 HP/SP、Buff、異常、戰鬥資源與技能 Timing，不扣主角背包／Zeny、不寫回分身雲端存檔；無 SQL 更新。

## V0.9.88B8

- 分身傭兵視窗新增每個傭兵獨立的「技能設定」入口與攻擊／輔助分頁。
- 輔助分頁只顯示該角色 Snapshot 中確實已學會、且已核准的 65 個群體 Buff／4 個群體恢復技能；可全選、全取消或逐招勾選。
- 技能選擇依角色保存在既有傭兵隊伍設定，不寫回分身角色、背包、貨幣或雲端角色存檔。
- 攻擊技能分頁與持久化欄位先完成；B9 已接入正式攻擊技能目錄與主動施放 AI。
- 完成全技能群體範圍複查：現行 69 個可執行友方群體技能沒有漏接；光之障壁仍依決議排除。
- 單體友軍技能另整理為可執行候選、專案改造後需恢復候選、Metadata-only 待實作三類。
- 完整保留 B7 群體效果、B6 草類多段固定傷害、B5 自動分解與既有 OOM 保護；無 SQL 更新。
