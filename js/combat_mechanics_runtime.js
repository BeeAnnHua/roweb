// RO_WEB 0.9.82EH - RA Renewal trait stats, critical C.RATE, ground placement and exact tick runtime
(function(){
"use strict";
...
const HitResolver={
 ...
 resolve(attacker,defender,opt={}){
  ...
  const perfectChance=clamp(Number(active.perfectHitRate||0),0,100);
...
const CriticalResolver={
 chance(attacker,defender,opt={}){
  ...
  let chance=cri-(luk*Number(opt.targetLukPenaltyPerPoint??2)/scale*100)+Number(opt.criticalRateBonus||0)+raceBonus;
...
const StatusManager={
 ...
 chance(target,status,base,opt={}){
  ...
  chance-=Math.floor(stat/5)+Number(target?.statusResist?.[id]||0)+activeResistance;
 ...
 apply(target,status,opt={}){
  ...
  if(target===window.player){
   ...
   const blocked=(whole&&[...groups.weapon,...groups.shield,...groups.armor,...groups.headgear].includes(normalized))
    ||(Number(active.weaponProtection||0)>0&&groups.weapon.includes(normalized))
    ||(Number(active.shieldProtection||0)>0&&groups.shield.includes(normalized))
    ||(Number(active.armorProtection||0)>0&&groups.armor.includes(normalized))
