...
function getRuntimeSkillTimingProfile(skill, level = 1) {
  ...
  const baseWalkDelayMs = ignoreWalkDelay || getRuntimeSkillUiType(skill) === "passive" ? 0 : 300 + explicitWalkDelayMs;
  ...
  return {
    ...
    explicitWalkDelayMs, afterCastWalkDelayMs,
  ...
  const effectId = window.GroundEffectManager.create({
    ...
    onTick(targets,effect){
      for (const target of (targets || []).slice(0,maxTargets)) {
        ...
        const knockbackCells = Math.max(0, Number(getLevelValue(profile.knockbackCells, level, 0)) || 0);
...
function castAttackSkill(skill, requestedLevel = null, options = {}) {
  ...
  for (const target of targets) {
    ...
    const knockbackCells=Number(getLevelValue(profile.knockbackCells,level,0));
  ...
  const effectId=window.GroundEffectManager.create({
    ...
    onTick(targets,effect,context){
      ...
      for(const target of selectedTargets){
        ...
        const knockbackCells=Math.max(0,Number(getLevelValue(profile.knockbackCells,level,0)));if(knockbackCells>0)window.MovementEffectResolver?.knockback(target,player,knockbackCells);
