//============================================================
// RO_WEB 0.9.82IL — 台灣頭飾 轉蛋單件絕對機率＋全域指定數量批量開啟橋接
...
.ro-taiwan-gacha-banner{box-sizing:border-box;width:100%;padding:11px 18px;border:2px solid rgba(255,255,255,.72);border-radius:10px;color:#fff;text-align:center;font-weight:800;letter-spacing:.03em;text-shadow:0 2px 3px rgba(0,0,0,.9);box-shadow:0 5px 20px rgba(0,0,0,.45);opacity:0;transform:translateY(-18px) scale(.98);animation:roTaiwanGachaBanner 4.8s ease forwards}
.ro-taiwan-gacha-banner.red{background:linear-gradient(90deg,rgba(132,0,0,.96),rgba(226,38,38,.96),rgba(132,0,0,.96))}
.ro-taiwan-gacha-banner.purple{background:linear-gradient(90deg,rgba(65,15,105,.97),rgba(151,63,214,.97),rgba(65,15,105,.97))}
.ro-taiwan-gacha-banner.gold{color:#2d1b00;background:linear-gradient(90deg,#b77700,#ffe47b,#d39500);text-shadow:0 1px 1px rgba(255,255,255,.65);border-color:#fff2a6}
@keyframes roTaiwanGachaBanner{0%{opacity:0;transform:translateY(-18px) scale(.98)}10%,82%{opacity:1;transform:translateY(0) scale(1)}100%{opacity:0;transform:translateY(-12px) scale(.99)}}
@media(max-width:640px){.ro-taiwan-gacha-banner{padding:9px 12px;font-size:14px;border-radius:8px}}
      `;
  ...
  function showRareBanner(tier, text) {
    ...
    banner.className = `ro-taiwan-gacha-banner ${["red","purple","gold"].includes(tier) ? tier : "red"}`;
  ...
  function flushGachaBatchUi(item, opened, summary, rareAnnouncements, options = {}) {
    ...
    if (!options.skipSave && !window.RO_WEB_SAVE_PREPARING_REWARDS) {
      if (typeof window.requestGameSave === "function") window.requestGameSave(1200, "taiwan-gacha-batch");
      else window.setTimeout(() => window.saveGame?.({ reason:"taiwan-gacha-batch", preparePendingRewards:false }), 0);
    }
  ...
  function processGachaBatch(options = {}) {
    ...
    if (!options.skipCheckpoint && GACHA_BATCH.openedSinceCheckpoint >= 256) {
      ...
      window.saveGame?.({ reason:"taiwan-gacha-checkpoint", durableDelayMs:0, preparePendingRewards:false });
    }
    ...
    if (GACHA_BATCH.pending <= 0 && opened > 0 && !options.skipSave && !window.RO_WEB_SAVE_PREPARING_REWARDS) {
      window.saveGame?.({ reason:"taiwan-gacha-final", durableDelayMs:0, preparePendingRewards:false });
    }
  ...
  function emitGachaBatchProgress(reason = "progress") {
    ...
    window.dispatchEvent(new window.CustomEvent("ro-web:batch-open-progress", {
      detail:{
        adapterId:"taiwan_gacha",
        itemId:Number(GACHA_BATCH.item?.id || gachaItemId()),
        itemName:String(GACHA_BATCH.item?.name || itemData(gachaItemId())?.name || "光輝天翼轉蛋"),
  ...
  function rollMapExclusiveDrop(monster) {
    ...
    if (state.mapExclusiveTaiwanGachaRolled) return false;
    state.mapExclusiveTaiwanGachaRolled = true;
  ...
  window.useItem = function taiwanGachaUseItem(itemId, instance = null, options = {}) {
  ...
  window.TaiwanGachaRuntime = Object.freeze({
    version:VERSION,
    config,
    rollReward,
    openGacha,
