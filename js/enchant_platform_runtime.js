...
(function(){
  ...
  function openEnchantPlatform(npc){
    ...
    setMessage(rows.length?"已偵測背包與穿戴中的黯淡冰晶武器。附魔固定依第4洞 → 第3洞 → 第2洞進行。":"目前沒有可附魔的黯淡冰晶武器。可先從蛇巢穴取得黯淡冰晶武器箱。" );
    window.bringWindowToFront?.(host);window.addBattleLog?.(`${npc?.name||"附魔研究員"}：已開啟正式黯淡冰晶附魔平台。`);return true;
  }
