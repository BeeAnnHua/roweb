// RO_WEB 0.9.82EH - RA Renewal damage formula + Trait P.ATK/S.MATK/C.RATE + universal physical element endow
(function(){
"use strict";
...
function finalModifiers(raw,target,opt={}){
 ...
 damage=window.applyROCombatDamageModifiers(damage,{damageType:opt.damageType||'physical',target,source:opt.source||window.player,attackElement:flags.ignoreElement?'Neutral':opt.element,
  ...
  mdefPiercePercent:opt.mdefPiercePercent,hardDef:opt.hardDef,softDef:opt.softDef,hardMdef:opt.hardMdef,softMdef:opt.softMdef,res:opt.res,mres:opt.mres,flatReduction:opt.flatReduction,simpleDefense:opt.simpleDefense,critical:opt.critical===true,minimumDamage:opt.minimumDamage,hitCount:Math.max(1,Number(opt.hitCount??opt.hits??1)||1)});
...
function resolveNormalAttack(target,opt={}){
 ...
 const parts=buildPhysicalParts({critical:crit.critical,atkRate:Number(opt.atkRate||0),masteryAtk:Number(opt.masteryAtk||0),profile:{elementSource:'weapon'},target,includeLeftHand:true});
...
function resolvePhysicalSkill(profile,level,target,opt={}){
 ...
 const parts=buildPhysicalParts({critical:damageCritical,atkRate:Number(profile.atkRate||0),masteryAtk:Number(profile.masteryAtk||0),profile,target,ignoreSize:profile.applyWeaponSize===false||flags.ignoreSize,includeLeftHand:profile.includeLeftHandDamage===true||profile.useLeftHandDamage===true});
