//============================================================
// RO_WEB 0.9.82HR — 裝備升階 Runtime／5% 絕對材料掉落
//============================================================
(() => {
  "use strict";
  const VERSION="0.9.82HR", RULE_KEY="data/enchant_grade_rules.json", EXCHANGE_KEY="data/enchant_grade_exchange.json", DROP_KEY="data/enchant_grade_map_drops.json";
  ...
  function getGradeMaterialDropChanceBasisPoints(){const raw=window.RO_WEB_DATA?.["data/server_config.json"]?.server?.rates?.gradeMaterialDropChanceBasisPoints ?? window.serverConfig?.server?.rates?.gradeMaterialDropChanceBasisPoints ?? window.serverConfig?.rates?.gradeMaterialDropChanceBasisPoints ?? 500;return Math.max(0,Math.min(10000,n(raw,500)));}
  function getScaledGradeDropChance(entryOrChance){const entry=entryOrChance&&typeof entryOrChance==="object"?entryOrChance:{chance:entryOrChance};const explicit=entry.absoluteChanceBasisPoints??entry.finalChanceBasisPoints;return Math.max(0,Math.min(10000,n(explicit,getGradeMaterialDropChanceBasisPoints())));}
  function getScaledMapDropChance(entryOrChance){const entry=entryOrChance&&typeof entryOrChance==="object"?entryOrChance:{chance:entryOrChance},mode=String(entry.rateMode||"").toLowerCase();if(mode==="normal"||mode==="globaldrop"||mode==="global_drop"){if(typeof window.getFinalDropChanceBasisPoints==="function")return window.getFinalDropChanceBasisPoints(entry.chance,"normal");const raw=Math.max(0,i(entry.chance));return Math.max(0,Math.min(10000,typeof window.applyRate==="function"?Math.floor(window.applyRate(raw,"drop")):raw));}return getScaledGradeDropChance(entry);}
  function rollMapBonusDrops(monster){const mapId=window.currentMap?.id||window.player?.map||"",profile=dropProfiles()?.profiles?.[mapId];if(!profile||!monster)return [];const awarded=[];for(const e of profile.entries||[]){if(Array.isArray(e.monsterIds)&&!e.monsterIds.includes(Number(monster.id)))continue;if(e.skipIfOriginalDrop&&[...(monster.drops||[]),...(monster.mvpDrops||[])].some(d=>itemId(d.itemId)===itemId(e.itemId)))continue;const chance=getScaledMapDropChance(e);if(chance<=0||Math.floor(Math.random()*10000)+1>chance)continue;const qty=Math.max(1,Math.floor(n(e.qtyMin,1)+Math.random()*(n(e.qtyMax,e.qtyMin)-n(e.qtyMin,1)+1)));window.addItem?.({id:e.itemId,name:itemName(e.itemId)},qty);window.recordItemDrop?.(e.itemId,qty);const normalMode=["normal","globaldrop","global_drop"].includes(String(e.rateMode||"").toLowerCase());window.RareItemAnnouncementRuntime?.announceAcquisition?.({itemId:e.itemId,itemName:itemName(e.itemId),quantity:qty,chanceBasisPoints:chance,source:normalMode?"map_bonus_drop":"grade_material_drop",sourceLabel:`${profile.mapName||mapId}${normalMode?"地圖掉落":"升階材料"}`});window.emitLootRewardLog?.(`${normalMode?"地圖掉落":"升階材料"}：額外取得 ${itemName(e.itemId)} ×${qty}。`,"item");awarded.push({itemId:e.itemId,qty,chance,rateMode:normalMode?"normal":"grade"});}return awarded;}
  ...
  async function open(npc,options={}){
    if(window.ROWebOfflineContinuity?.isOffline?.())return window.ROWebOfflineContinuity.guard("enchant","裝備升階／附魔");
