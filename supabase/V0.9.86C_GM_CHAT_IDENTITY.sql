...
comment on column public.ro_chat_messages.sender_role is
  'V0.9.86C server-verified sender identity snapshot used only for chat cosmetics; currently player/gm.';
comment on function public.ro_chat_poll(uuid,bigint,integer) is
  'V0.9.86C low-traffic incremental chat poll; includes sender_role without extra requests.';
