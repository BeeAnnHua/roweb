...
check(usedBonusTypes.size===145,'Static rAthena bonus vocabulary count',usedBonusTypes.size);
...
check(evaluations===29920,'Full effect matrix evaluation count',evaluations);
...
// 10. FZ equipment grade, RES/MRES and Critical Rate bridge.
resetPlayer();
const ltBootOut=CardRuntime._debugEvaluateRecord(equipment['470183'],{sourceType:'equipment',hostRow:{itemId:470183,refine:13,grade:4,instance:{enchantGrade:4},item:{id:470183}},maxRefine:13,maxGrade:4,equippedIds:[470183]});
check(!ltBootOut.runtimeError&&ltBootOut.resFlat===30&&ltBootOut.mresFlat===30&&ltBootOut.pAtk>=7&&ltBootOut.sMatk>=7,'LT enchant grade D/C/B/A effects compile and emit RES/MRES/P.ATK/S.MATK',JSON.stringify({error:ltBootOut.runtimeError,res:ltBootOut.resFlat,mres:ltBootOut.mresFlat,pAtk:ltBootOut.pAtk,sMatk:ltBootOut.sMatk}));
const temporalBootOut=CardRuntime._debugEvaluateRecord(equipment['22202'],{sourceType:'equipment',hostRow:{itemId:22202,refine:10,grade:0,instance:{enchantGrade:0},item:{id:22202}},maxRefine:10,equippedIds:[22202]});
check(!temporalBootOut.runtimeError&&temporalBootOut.criRate===30,'bCriticalRate flows to the status criRate consumer',JSON.stringify({error:temporalBootOut.runtimeError,criRate:temporalBootOut.criRate}));
const coverageFz=EffectRuntime.auditCanonicalSource({...ltBootOut,...temporalBootOut});
check(coverageFz.missing.length===0,'GA inherited canonical status keys have named consumers',JSON.stringify(coverageFz.missing));

// 11. Explicit source audit and fail-loud future command diagnostic.
...
fs.writeFileSync(path.join(ROOT,'tools/test_effect_runtime_report_0.9.82GR.json'),JSON.stringify(report,null,2)+'\n');
