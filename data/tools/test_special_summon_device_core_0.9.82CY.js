...
assert.strictEqual(gen.version,'0.9.82CY');
...
assert.strictEqual(core.skills['243'].description,'RO_WEB 暫行效果：每級永久增加 ATK／MATK 傷害 2%。未來建立生命體 AI 後再接回召喚功能。');
assert.strictEqual(core.skills['244'].description,'RO_WEB 暫行效果：每級永久增加 ATK／MATK 傷害 2%。未來建立生命體 AI 後再接回安息功能。');
assert.strictEqual(core.skills['247'].description,'RO_WEB 暫行效果：每級永久增加 ATK／MATK 傷害 2%。未來建立生命體 AI 後再接回復活功能。');
...
(async()=>{await ctx.loadVirtualSummonData();
 ...
 console.log(JSON.stringify({result:'PASS',plantDamageWithMastery:6600,marineSphereDamageWithMastery:4400,mainSummonCoexists:true,independentSummonTypes:Object.keys(cfg.independentSummons).length,homunculusSkillsUntouched:[243,244,247]},null,2));
