...
check(config.get('version')=='0.9.82EV','spawn config version')
check(manifest.get('version')=='0.9.82EV','manifest version')
...
check(ai_modes.get('version')=='0.9.82EV','AI mode version')
...
check(versions=={'0.9.82EV'},f'cache versions {versions}')
check('RO_WEB V0.9.82EV' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EV','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
