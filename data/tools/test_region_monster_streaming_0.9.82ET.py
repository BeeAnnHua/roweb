...
check(config.get('version')=='0.9.82ET','spawn config version')
check(manifest.get('version')=='0.9.82ET','manifest version')
...
check(ai_modes.get('version')=='0.9.82ET','AI mode version')
...
check(versions=={'0.9.82ET'},f'cache versions {versions}')
check('RO_WEB V0.9.82ET' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82ET','status':'PASS','raAiModes':True,'regions':10,'selectedSpecies':len(all_ids),
