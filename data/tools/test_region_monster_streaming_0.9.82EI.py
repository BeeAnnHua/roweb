...
check(config.get('version')=='0.9.82EI','spawn config version')
check(manifest.get('version')=='0.9.82EI','manifest version')
...
for token in ['monster_spawn_config.json','monsterCountRate','normalSpawnDelayRate','plantSpawnDelayRate','bossSpawnDelayRate','activeWindowSourceSize','retainWindowSourceSize','normalHardCap','respawnQueue','nextSpawnAt','frame.atlas','animationAtlases','onWorldMonsterDefeated','getWorldMonsterTestEntities']:
...
check('V0.9.82EI - RA regional monster streaming' in css,'stream CSS missing')
check('.world-monster-debug-cross' not in css,'debug cross CSS remains')
index=(ROOT/'index.html').read_text(encoding='utf-8')
versions=set(re.findall(r'[?&]v=([^"&]+)',index))
check(versions=={'0.9.82EI'},f'cache versions {versions}')
check('RO_WEB V0.9.82EI' in index,'title version')
...
print(json.dumps({
  'version':'0.9.82EI','status':'PASS','regions':10,'selectedSpecies':len(all_ids),
