...
assert(counts.pending===312,`pending count ${counts.pending}`);
assert(counts.passive===216,`passive count ${counts.passive}`);
assert(activeEligible===611,`active eligible ${activeEligible}`);
...
console.log(JSON.stringify({version:'0.9.82DW',status:'PASS',counts,activeQuickSlotEligible:activeEligible,legacyTypeDrift:legacyDrift},null,2));
