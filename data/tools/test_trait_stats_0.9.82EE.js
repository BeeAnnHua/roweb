...
context.player.statusAllocationStep = 1;
context.toggleStatusAllocationStep();
assertEq(context.getStatusAllocationStep(), 10, 'Global allocation mode toggles to +10');
assertEq(context.allocateTraitPoints('pow', context.getStatusAllocationStep()), 10, 'Global +10 allocates Trait points');
assertEq(context.player.traits.pow, 10, 'Trait +10 stored');
assertEq(context.getAvailableTraitPoints(), 567, 'Available after global +10 allocation');
context.toggleStatusAllocationStep();
assertEq(context.getStatusAllocationStep(), 1, 'Global allocation mode toggles back to +1');
assertTrue(context.allocateTraitPoint('pow'), 'Allocate Trait +1');
assertEq(context.player.traits.pow, 11, 'Trait +1 stored');
context.player.traits.pow = 107;
context.player.traitStats = context.player.traits;
context.normalizeStatusData();
assertEq(context.allocateTraitPoints('pow', 10), 3, '+10 clamps to per-trait cap');
assertEq(context.player.traits.pow, 110, 'POW clamped to 110');
...
for (const token of ['status-trait-toggle','status-trait-panel','status-allocation-step-button','status-advanced-toggle','status-advanced-panel','createAdvancedDualDetailRow','getTraitBreakdownTooltip','四轉 Job 加成','裝備／卡片','被動技能','目前 Buff']) {
  assertTrue(statusSource.includes(token), `UI source includes ${token}`);
}
assertTrue(!statusSource.includes('className = "status-trait-plus-ten"'), 'Per-row Trait +10 button removed');
const updateUiSource = statusSource.slice(statusSource.indexOf('function updateStatusUI()'), statusSource.indexOf('window.getTraitLevelPointsForLevel'));
assertTrue(!updateUiSource.includes('免費重置特性'), 'Free reset control/description hidden from current UI');
const styleSource = fs.readFileSync('css/style.css', 'utf8');
for (const token of ['.status-trait-toggle','.status-trait-panel','.status-trait-plus','.status-allocation-step-button','.status-advanced-panel','.status-advanced-tabs','.status-advanced-detail-row']) {
...
assertTrue(bundleSource.includes('data/trait_combat_policy.json'), 'Data bundle includes trait combat policy');
...
console.log('PASS 0.9.82EE rAthena Renewal trait stats, Lv200 fourth-job bonus, doubled level points, allocation and critical formula');
